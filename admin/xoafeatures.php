...
// Kiểm tra xem Đặc Tính Phòng đã được sử dụng trong phòng hay chưa
$query_check_usage = "SELECT COUNT(*) AS count FROM room_features WHERE features_id = $Id";
...
if ($count > 0) {
    ...
    echo "<script>alert('Không thể xóa Đặc Tính Phòng đã được sử dụng trong phòng.');</script>";
} else {
    // Xóa Đặc Tính Phòng nếu không có phòng sử dụng
    $query_delete = "DELETE FROM features WHERE Id = $Id";
